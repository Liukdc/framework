...
function checkConstitutionReferences() {
  try {
    ...
    const loaderExists = readFileSync(join(constitDir, 'loader.js'), 'utf-8').includes('loadAllConstitutions');
    const indexExists = readFileSync(join(L3_DIR, 'constitutions', 'index.json'), 'utf-8').includes('constitutionFiles');
    return {
      check: 'constitution-references',
      pass: loaderExists && indexExists,
      detail: '宪法加载器和索引均存在',
    };
  } catch {
