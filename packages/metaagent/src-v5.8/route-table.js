...
export class RouteTable {
  ...
  _matchFrom(actualFrom, patternFrom) {
    ...
    // IN_SESSION(topic) 匹配 IN_SESSION* 的规则
    if (patternFrom.startsWith('IN_SESSION(') && actualFrom.startsWith('IN_SESSION(')) {
      const patternType = patternFrom.match(/IN_SESSION\((\w+)/)?.[1];
      const actualType = actualFrom.match(/IN_SESSION\((\w+)/)?.[1];
      if (patternType && actualType) {
        if (patternType === 'topic' && !['N11', 'N12'].includes(actualType)) return true;
        if (patternType === 'N11,N12' && ['N11', 'N12'].includes(actualType)) return true;
      }
    }
    // 纯 IN_SESSION 匹配所有 IN_SESSION(*) 子类型
