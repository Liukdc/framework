...
const DET_TOOLS = {
  ...
  n2InjectRole1: {
    ...
    handler: async (args, ctx) => {
      ctx._n2Role1Output = args.sceneDefinition;
  ...
  n2InjectRole2: {
    ...
    handler: async (args, ctx) => {
      ctx._n2Role2Context = { role1Output: args.role1Output, n1Boundary: args.n1Boundary };
...
const OPTIONAL_TOOLS = {
  ...
  searchOutputs: {
    ...
    handler: async (args, ctx) => {
      return ctx.contractStore.searchConversations(args.query);
    },
  },
...
export class ToolRegistry {
  ...
  getToolDefinitions(intent) {
    const defs = [];
    // N5/N6/N7/N9/N11/N12 可调用全部工具
    // 其他环节只给必用工具
    const fullToolIntents = ['N5', 'N6', 'N7', 'N9', 'N11', 'N12', 'N13'];

    for (const [name, tool] of this._byName) {
      if (fullToolIntents.includes(intent) || tool.required) {
        defs.push({
          type: 'function',
          function: {
            name: tool.name,
