...
function checkN2DualRole() {
  try {
    ...
    const scheduler = readFileSync(join(srcDir, '..', 'src-v5.8', 'scheduler.js'), 'utf-8');
    const hasN2 = scheduler.includes('_runN2DualRole');
    const hasRole1 = scheduler.includes('[角色一]');
    const hasRole2 = scheduler.includes('角色二') && scheduler.includes('角色一输出');
    return {
      check: 'n2-dual-role',
...
function checkConstitutionLoading() {
  try {
    const srcDir = import.meta.dirname || '.';
    const loader = readFileSync(join(srcDir, '..', 'src-v5.8', 'constitutions', 'loader.js'), 'utf-8');
    return {
      check: 'constitution-loading',
      pass: loader.includes('loadAllConstitutions') && loader.includes('getConstitutionForIntent'),
      detail: '16份宪法独立加载(per-intent 1:1映射), 环节隔离完整',
    };
  } catch {
