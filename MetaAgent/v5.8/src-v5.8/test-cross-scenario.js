...
const L3 = {
  ...
  'states.json': {
    ...
    inSessionSubtypes: [
      { intent: 'record', taskType: 'field_based', topicEvolutionEnabled: false, importance: 'high' },
      { intent: 'query',  taskType: 'topic_based', topicEvolutionEnabled: true, importance: 'normal' },
      { intent: 'other',  taskType: 'topic_based', topicEvolutionEnabled: true, importance: 'normal' },
    ],
  },
  'transitions.json': [{ from: 'ANALYZING', trigger: ['intent'], to: 'IN_SESSION' }],
  'routeTable.json': {
    routes: [
